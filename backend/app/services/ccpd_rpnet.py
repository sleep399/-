...
def _decode_pass(
    ...
    return _PassResult(
        ...
        bbox=bbox_from_rpnet_box(box, orig_w, orig_h),
    )
...
class CCPDRPNetRecognizer:
    ...
    def _evaluate_image(self, image_bgr: np.ndarray) -> _PassResult | None:
        orig_h, orig_w = image_bgr.shape[:2]
        best: _PassResult | None = None
        for _, variant in _build_variants(image_bgr):
            logits, box, _, _ = self._forward(variant)
            result = _decode_pass(logits, orig_w, orig_h, box)
    ...
    def _locate_bbox_ccpd(self, image_bgr: np.ndarray) -> list[int]:
        """CCPD demo.py 定位：resize 到 480x480，box 映射回原图。"""
        orig_h, orig_w = image_bgr.shape[:2]
        stretched = cv2.resize(image_bgr, IMG_SIZE)
        logits, box, _, _ = self._forward(stretched)
        return bbox_from_rpnet_box(box, orig_w, orig_h)

    def recognize(self, image_bgr: np.ndarray, ccpd_bbox: list[int] | None = None) -> dict[str, Any]:
        if image_bgr is None or image_bgr.size == 0:
            return {"plate_number": "", "confidence": 0.0, "bbox": [0, 0, 0, 0], "indices": []}

        img_h, img_w = image_bgr.shape[:2]
        best = self._evaluate_image(image_bgr)

        if best and best.confidence < 0.85:
            seen: set[tuple[int, int, int, int]] = set()
            seed_bbox = self._locate_bbox_ccpd(image_bgr)
            for bbox in [seed_bbox] + detect_plate_regions(image_bgr):
                ...
                if crop.size == 0 or crop.shape[0] < 20 or crop.shape[1] < 40:
                    continue
                crop_best = self._evaluate_image(crop)
        ...
        rpnet_bbox = self._locate_bbox_ccpd(image_bgr)
        bbox = locate_plate_bbox(image_bgr, rpnet_bbox, ccpd_bbox=ccpd_bbox)

        x1, y1, x2, y2 = bbox
        roi = image_bgr[max(0, y1):y2, max(0, x1):x2]
        plate_color = detect_plate_color(roi) if roi.size > 0 else "蓝牌"
        plate_number = refine_one_vs_i(plate_number, roi, plate_color)
