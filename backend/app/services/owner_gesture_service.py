...
class OwnerGestureService:
    ...
    def _classify(self, landmarks, w, h) -> tuple[str, float]:
        ...
        finger_tips_y = [lm[8].y, lm[12].y, lm[16].y, lm[20].y]
        finger_pips_y = [lm[6].y, lm[10].y, lm[14].y, lm[18].y]
        fingers_up = sum([tip < pip for tip, pip in zip(finger_tips_y, finger_pips_y)])
        ...
        if fingers_up >= 4 and thumb_up:
            gesture, conf = "palm_open", 0.9
        elif fingers_up == 0 and not thumb_up:
            gesture, conf = "fist", 0.85
        elif thumb_up and fingers_up == 0:
            gesture, conf = "thumb_up", 0.8
        elif lm[4].y > lm[3].y and fingers_up == 0:
            gesture, conf = "thumb_down", 0.75
        ...
        if len(self._position_history) >= 8:
            ...
            thresh = w * 0.08
            if dx < -thresh:
                gesture, conf = "swipe_left", 0.8
            elif dx > thresh:
                gesture, conf = "swipe_right", 0.8
        ...
        if len(self._circle_points) >= 15:
            ...
            if avg_radius > w * 0.04 and max(dists) - min(dists) < w * 0.05:
                gesture, conf = "circle", 0.75

        if len(self._position_history) >= 5 and fingers_up >= 3:
            ys = [p[1] for p in list(self._position_history)[-5:]]
            y_range = max(ys) - min(ys)
            if y_range > h * 0.1:
                gesture, conf = "wave", 0.7
    ...
    def _apply_debounce(self, gesture: str, confidence: float) -> tuple[str, float, str | None]:
        ...
        most_common, count = Counter(self._gesture_history).most_common(1)[0]
        stable_gesture = most_common if count >= 4 else "no_gesture"

        if stable_gesture != gesture:
            return "no_gesture", confidence, None

        if self._gesture_start == 0.0:
            self._gesture_start = now
            return "no_gesture", confidence, None

        held_sec = now - self._gesture_start
        if held_sec < settings.gesture_hold_threshold:
            return "no_gesture", confidence, None

        if now - self._last_action_time < 1.0 and self._last_gesture == gesture:
            return "no_gesture", confidence, None
