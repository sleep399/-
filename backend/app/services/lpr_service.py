...
PLATE_ANNOTATION_COLORS = {
    "蓝牌": (255, 128, 0),    # BGR 蓝
    "绿牌": (0, 200, 0),      # BGR 绿
    "黄牌": (0, 220, 255),    # BGR 黄
    "白牌": (240, 240, 240),  # BGR 白
    "黑牌": (80, 80, 80),     # BGR 深灰
    "未知": (0, 255, 0),
}
...
class LicensePlateService:
    ...
    def _recognize_with_rpnet(self, image: np.ndarray, ccpd_bbox: list[int] | None = None) -> dict[str, Any]:
        result = self.rpnet.recognize(image, ccpd_bbox=ccpd_bbox)
        ...
        plate_color = result.get("plate_color") or (
            detect_plate_color(roi) if roi.size > 0 else "蓝牌"
        )
        return {
    ...
    def recognize(self, image_bytes: bytes, filename: str = "") -> dict[str, Any]:
        ...
        try:
            ccpd_bbox = ccpd_info["bbox"] if ccpd_info else None
            rpnet_result = self._recognize_with_rpnet(image, ccpd_bbox=ccpd_bbox)
        ...
        if ccpd_info:
            ...
            bbox = ccpd_info["bbox"] if ccpd_info else rpnet_result["bbox"]
            if len(plate_num) < 7:
            ...
            plates.append({
                ...
                "plate_color": detect_plate_color(roi) if roi.size > 0 else ccpd_info["plate_color"],
        ...
        for p in plates:
            x1, y1, x2, y2 = p["bbox"]
            box_color = PLATE_ANNOTATION_COLORS.get(p["plate_color"], (0, 255, 0))
