...
| `user_id` | INTEGER | 是 | NULL | 逻辑关联 `users.id` | 已确认登录的用户 |
| `created_at` | DATETIME | 是 | 当前 UTC 时间 | — | 创建时间 |
...
erDiagram
    ...
    USERS o|--o{ WECHAT_LOGIN_SESSIONS : "确认登录"
...
| `wechat_login_sessions` | `session_id` 唯一索引 |
